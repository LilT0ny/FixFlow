...
export const SearchInput: React.FC<SearchInputProps> = ({
  ...
  return (
    <div className={cn("relative w-full md:w-[360px] group", className)}>
      <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-surface-400 dark:text-gray-500 w-4 h-4 group-focus-within:text-primary-600 transition-colors duration-150" />
      <input
        type="text"
      ...
      {value && (
        <button
          onClick={() => onChange('')}
          className="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded text-surface-400 hover:text-surface-600 dark:text-gray-500 dark:hover:text-gray-300 transition-colors duration-150 animate-fade-in"
        >
          <X className="w-4 h-4" />
        </button>
      )}
