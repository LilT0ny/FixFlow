          ...
          <table className="w-full text-left min-w-[600px] md:min-w-0">
            <thead>
              <tr className="bg-surface-50/50 text-[10px] font-black text-surface-400 uppercase tracking-[0.25em] border-b border-surface-100">
                ...
                <th className="px-6 md:px-8 py-5 hidden sm:table-cell">Detalle</th>
                  ...
                  <td className="px-6 md:px-8 py-6 text-center">
                    <div className="flex justify-center gap-2">
                       <button
                        ...
                        className="w-9 h-9 md:w-10 md:h-10 flex items-center justify-center text-primary-600 bg-primary-50 hover:bg-primary-600 hover:text-white rounded-xl md:rounded-2xl transition-all shadow-sm active:scale-95"
                      ...
                      <button
                        ...
                        className="w-9 h-9 md:w-10 md:h-10 flex items-center justify-center text-rose-600 bg-rose-50 hover:bg-rose-600 hover:text-white rounded-xl md:rounded-2xl transition-all shadow-sm active:scale-95"
          ...
          <div className="bg-white rounded-[24px] md:rounded-[40px] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)] w-full max-w-4xl max-h-[90vh] flex flex-col animate-zoom-in overflow-hidden border border-white/20">
            ...
            <div className="px-6 md:px-10 py-6 md:py-8 border-b border-surface-50 bg-surface-50/30 flex justify-between items-center">
              <div className="flex items-center gap-4 md:gap-5">
                <div className="w-12 h-12 md:w-14 md:h-14 bg-primary-600 text-white rounded-[16px] md:rounded-[20px] shadow-xl shadow-primary-200/50 flex items-center justify-center shrink-0">
                  <ShoppingBag className="w-6 h-6 md:w-7 md:h-7" />
                </div>
                <div>
                  <h3 className="text-xl md:text-2xl font-black text-surface-900 tracking-tight leading-none">Nueva Transacción</h3>
                  <p className="text-[9px] md:text-[10px] font-black text-surface-400 uppercase tracking-[0.2em] mt-2 opacity-60">Generación de Nota de Venta Directa</p>
                </div>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4 md:space-y-6">
                  {newSale.items.map((item, idx) => (
                    <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-5 p-4 md:p-6 bg-surface-50 rounded-[24px] md:rounded-[28px] border border-surface-100 transition-all hover:bg-white hover:border-primary-200 hover:shadow-xl hover:shadow-primary-100/20 group animate-in slide-in-from-right-4 duration-300">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 col-span-1 md:col-span-4 gap-4">
                      ...
                      <div className="col-span-1 md:col-span-1 flex items-end justify-center md:pb-2">
                        <button 
                          onClick={() => handleRemoveItem(item.id)}
                          disabled={newSale.items.length === 1}
                          className="w-full md:w-10 h-10 flex items-center justify-center text-rose-300 hover:text-white hover:bg-rose-500 disabled:opacity-30 rounded-xl transition-all shadow-sm border border-surface-100 md:border-none"
                        >
                          <Trash2 className="w-4 h-4" />
                          <span className="md:hidden ml-2 text-[10px] font-black uppercase">Eliminar Ítem</span>
                        </button>
                      </div>
                    </div>
                  ))}
            ...
            {/* Footer / Acción */}
            <div className="px-6 md:px-10 py-6 md:py-8 border-t border-surface-50 bg-surface-50/30 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-6">
              <div className="flex items-center gap-4 md:gap-6 border-b sm:border-none pb-4 sm:pb-0">
                <div className="flex flex-col">
                  <span className="text-[9px] md:text-[10px] font-black text-surface-400 uppercase tracking-widest">Total Consolidado</span>
                  <span className="text-3xl md:text-4xl font-black text-surface-900 leading-none tracking-tighter mt-1">
                    ${total.toFixed(2)}
                  </span>
                </div>
                
                <div className="hidden sm:block h-10 w-[1px] bg-surface-200" />
                
                <div className="flex flex-col">
                   <span className="text-[9px] md:text-[10px] font-black text-surface-400 uppercase tracking-widest">Medio de Pago</span>
                   <select 
                    className="mt-1 bg-transparent border-none text-[10px] md:text-[11px] font-black text-primary-600 focus:ring-0 uppercase cursor-pointer p-0"
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-3 md:gap-4">
                <button
                  ...
                  className="px-6 md:px-8 py-3.5 md:py-4 border border-surface-200 text-surface-500 rounded-xl md:rounded-2xl font-black text-[10px] md:text-[11px] uppercase tracking-widest hover:bg-surface-100 transition-all active:scale-95"
                >
                  Regresar
                </button>
                <button
                  ...
                  className="px-8 md:px-10 py-3.5 md:py-4 bg-primary-600 text-white rounded-xl md:rounded-2xl font-black text-[10px] md:text-[11px] uppercase tracking-widest hover:bg-primary-700 transition-all shadow-2xl shadow-primary-200/50 flex items-center justify-center gap-3 disabled:opacity-50 active:scale-95"
                >
                  {saveStatus === 'saving' ? (
